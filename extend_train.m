function [ind_TAB,ind_TAB_new,ind_new]=extend_train(Labels,classes,OPT_extend)
% Extend training set for class imbalance 
% for all classes with N < helf of the median
if(nargin<3),OPT_extend=0;end
ind_max_ref=3;    %3rd maximum       --- old: 2nd 
if(OPT_extend>0),ind_max_ref=numel(classes)-1; end
fprintf(' Extend Training set - initial Labels:%6.0f   n.classes:%6.0f\n',numel(Labels),numel(classes));
...
for ii=find(TAB_ini>0)
    ...
    if(TAB_ini(ii)<maxtab(ind_max_ref))
        KK2=(maxtab(ind_max_ref)/TAB_ini(ii))-1;fprintf(' * %6.2f',KK2);
    else
        KK2=0;
        fprintf('        .');
    end
