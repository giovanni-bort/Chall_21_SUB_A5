...
load ECG2_12
K_pass_sample=2; K_pass_leads=0.1;
...
%fprintf(' t:%8.1f  mean:%8.1f',T_2-T_1,T_2/(ii-K_ini+1))
 ...
 T2_1=toc;
  ...
  [IM_XM,IM_YM]=ndgrid(1:K_pass_sample:NN,0.5:K_pass_leads:12.5);
  View_ECG_im=F_F(IM_XM,IM_YM)';  
  IM3=imagesc(View_ECG_im);   %   view image
T2_2=toc;
%fprintf(' t:%8.1f  mean:%8.1f',T_2-T_1,T_2/(ii-K_ini+1))
     fprintf(' ---elapsed time: %10.2f\n',T2_2-T2_1);
fprintf('sum 1: %15.2f\n',sum(sum(abs(VVq))));
fprintf('sum 2: %15.2f\n',sum(sum(abs(View_ECG_im))));
fprintf('diff : %15.2f\n',sum(sum(View_ECG_im-VVq)));
