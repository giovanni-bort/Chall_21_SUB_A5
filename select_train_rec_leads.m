function [list_ok,files_ok,full_files_ok,List_Diagn]=select_train_rec_leads(dir_IMG,CHK_leads,n_MAX_rec,n_DIA_chk,NORM_chk,opt_prn)
%-------INPUT:---------
...
%  **********************************************
% **** TOLGO I NORMALI CON ALTRE DIAGNOSI ********
%  **********************************************

if(nargin<3),  n_MAX_rec=0; end
if(nargin<4),  n_DIA_chk=0; end
if(nargin<5),  NORM_chk=0;  end       %NORM_chk : 1: tolgo NORMAL se presenti altre diagnosi
if(nargin<6),  opt_prn=1;   end
...
IMG_files=dir([dir_IMG filesep CHK_leads '_*.jpg']);
n_files=numel(IMG_files);
if(opt_prn>0), fprintf(' %s -> presenti %6.0f files\n',[dir_IMG filesep  CHK_leads],n_files);end
...
% ECG_XNNNNN_DDDD.jpg
% ECG_02L_XNNNNN_DDDD.jpg
% ECG_03L_XNNNNN_DDDD.jpg
% ECG_06L_XNNNNN_DDDD.jpg
% ECG_12L_XNNNNN_DDDD.jpg
ID1=0;ID2=0;ID3=0;ID4=0;
N_rec=str2num(stringa(10:14));   % old: 6:10
typ_DB=stringa(9);               % old: 5
N_DB=0;
...
k=0;
for i=16:2:length(stringa)-4     % old: 12:2:length
    k=k+1;
    diagn(k)=str2num(stringa(i:i+1));
end
if(k==0),fprintf('K==0*** %s \n',stringa);end
